...
const stad = "https://thomasnass-app.herokuapp.com/city";
const land = "https://thomasnass-app.herokuapp.com/country";
            ...
            for (i = 0; i < data2.length; i++) {
                for (j = 0; j < visited.length; j++) {
                    if (data2[i].id == visited[j]) {
                        ...
                        li.textContent = `Du har besökt ${data2[i].cityname}`;
...
const renderCityBtns = (countryButton, data2) => {
    ...
    for (i = 0; i < data2.length; i++) {
        if (countryButton.id == data2[i].countryid) {
            ...
            cityButton.id = `${data2[i].cityname}`;
            cityButton.textContent = `${data2[i].cityname}`;
...
const renderCityInfo = (cityButton, data2) => {
    erase(onDisplay);
    for (i = 0; i < data2.length; i++) {
        if (cityButton.id == data2[i].cityname) {
            const p = document.createElement("p");
            const storeCityButton = document.createElement("button");
            storeCityButton.id = `${data2[i].cityname}${data2[i].id}`
            storeCityButton.textContent = "Lägg till stad som besökt";
            p.textContent = `Antalet invånare i ${data2[i].cityname} är : ${data2[i].population} `;
...
const storeInLS = (storeCityButton, data2) => {
    for (i = 0; i < data2.length; i++) {
        if (storeCityButton.id == `${data2[i].cityname}${data2[i].id}`) {
            if (localStorage.getItem("visited")) {
                visited = JSON.parse(localStorage.getItem("visited"));
            }
